...
"""
	This is the base class that from which all methods of inspecting a record are derived from
	The Inspect subclasses will assume the sample that is provided has already been vetted and thus is representative and delimited appropriately
"""
class Inspect(Thread):
	"""
		@param sample	list of rows, each row is a vector (note a list of lists is a matrix)
	"""
	...
	"""
		This function is design to assert a particular row. The assertion is returned as a binary vector designated fields that pass and others that don't
		@param row	a row of a file
	"""
	def inspect(self,row):
		pass
	"""
		This function is designed to convert the sample into usable format depending on the field inspection method implemented
		By default it will only return the sample and must be overriden by base classes
	"""
	def convert(self,sample):
		return sample
"""
	The idea behind assessing a length from a sample will simply consist in determining compute the average length and the variance
	If the variance is small enough we can assume the length of the field is equal to it's average.
"""
class InspectFieldLength(Inspect):
	...
	"""
		This function is designed to put the sample data into usable form : in our case every column will be converted into the length
	...
	def run(self):
		"""
			...
			The computation of column based sum/variance is performed by transposing the data structure because the loop operates on a row basis
...
"""
	This class is designed to inspect numeric types, this will include integers and doubles
	@TODO: We have to write perhaps more specific integer assessment and double assessment to be used 
"""
...
"""
	This class is designed to inspect date types in 3 formats formats yyyy-dd-mm|dd-mm-yyyy|dd-M-yyyy
	The confirmation of data should be enforced by the length. We assume/conjecture that a dataset has consistent output when it comes to dates (hopefully)
"""
class InspectDateField(InspectFieldType):
	def __init__(self,sample):
		Inspect.__init__(self,sample) ;
	"""
		This function will return a regex date pattern to be used to identify dates
	"""
	def getPattern(self):
		return '(^\d{1,4}-\d{1,2}-\d{1,2}|^\d{1,2}-\d{1,2}-\d{2,4} | \d{1,2}[A-Z][a-z]-\d{1,4} )'
		
"""
	This class is designed to sample the content and derive basic information upon which meaningful information can be derived (estimating population parameters)
	Method:
		- Estimate the size of the file in terms of number of rows/lines and extract a fifth (20%) as sample to determine number of columns & delimiter.
		- If the first assumption holds we can derive more meaningful statistics from this. 
		@TODO:
			Incorporate a mechanism like (bootstrap) where various sizes of fractions are tried in order to have confirmation on the basic assumption in order to yield better results
"""
class SampleBuilder(Thread):
	...
	"""
		...
		The function is called before delimiter/number od columns are determined. After delimiter is delimited and number of columns are determined, the read will insure the sample meets the criteria
		
		@param path	path to the file to sample
		@param size	size of the sample to be read
	"""
	...
	"""
		This function is designed to clean a row of data by removing non-ascii character. 
		The submitted row can either be a set of columns or a string and it will return the row in the type in which it was received.
		
		@pre		self.xchar != None and len(sample) > 0
		@param row	row to be cleaned
	"""
	def clean (self,row):
	...
	"""
		...
		The assumption we have made is based upon the central limit theorem i.e marginal delimiters will have smaller columns
	"""
...
class Disk(Output):
	...
	"""
		...
		@param id	identifier {passed,fixed,broken,log}
...
class Filter(Thread):
	...
	def run(self):
		...
		#
	"""
		This function is designed to log unfit records with records that will be ignored
	"""
